...
export const ACHIEVEMENTS: AchievementSpec[] = [
  ...
  // ---------- 1.0+ 32 角色扩展（新增常规角色解锁；全部复用现有 AchView 字段） ----------
  { id: 'survive10', icon: 'icon_dagger', unlockChar: 'willow',
    check: (v) => (v.run?.time ?? 0) >= 600 },
  { id: 'kills750', icon: 'icon_axe', unlockChar: 'samara',
    check: (v) => v.stats.kills >= 750 },
  { id: 'level35', icon: 'icon_fireball', unlockChar: 'cinder',
    check: (v) => (v.run?.level ?? 0) >= 35 },
  { id: 'evolve4', icon: 'icon_flask', unlockChar: 'tidey',
    check: (v) => (v.run?.evolves ?? 0) >= 4 },
  { id: 'evolve5', icon: 'icon_bolt', unlockChar: 'ray',
    check: (v) => (v.run?.evolves ?? 0) >= 5 },
  { id: 'arcanaDuo', icon: 'icon_bird', unlockChar: 'pipit',
    check: (v) => (v.run?.arcana ?? 0) >= 2 },
  { id: 'endless1', icon: 'icon_ricochet', unlockChar: 'beanie',
    check: (v) => (v.run?.endlessCycle ?? 0) >= 1 },
  { id: 'endless2', icon: 'icon_wand', unlockChar: 'wish',
    check: (v) => (v.run?.endlessCycle ?? 0) >= 2 },
  { id: 'hyperClear2', icon: 'icon_breath', unlockChar: 'pollen',
    check: (v) => v.run?.win === true && v.run.difficulty >= 2 },
  { id: 'wins15', icon: 'icon_gravity', unlockChar: 'vorty',
    check: (v) => v.stats.wins >= 15 },
  { id: 'charWins10', icon: 'icon_sword', unlockChar: 'lancey',
    check: (v) => (v.stats.charWins ?? 0) >= 10 },
  { id: 'affixHunter50', icon: 'icon_swarm', unlockChar: 'beebee',
    check: (v) => (v.stats.affixKills ?? 0) >= 50 },
  { id: 'flawlessHyperBoss', icon: 'icon_frost', unlockChar: 'frosty',
    check: (v) => v.run?.win === true && v.run.difficulty >= 1 && v.run.bossNoHit },
  { id: 'fullBuildClear', icon: 'icon_tornado', unlockChar: 'twirl',
    check: (v) => v.run?.win === true && v.run.weapons >= 6 && v.run.passives >= 6 },
];
