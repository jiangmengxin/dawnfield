...
- 初始武器来自 `weapon` 字段；32 位角色与 32 把基础武器一一配对，其中 `blobby` 和 `nova` 是隐藏角色。
- 部分角色通过 `trait` 获得机制差异，行为在 `systems/traits.ts`，数值集中在 `TRAIT_FX`。
...
- 每图至少 3 个地图专属掉落道具，其中至少 1 个带 `tags: ['lethal']` 的强杀伤道具；均通过机制产物调用 `ctx.spawnMapDrop`。
...
Boss 配装在 `src/content/bosses.ts`。每个 Boss 必须提供独占主招 + 轻辅招，二阶段统一半血触发并改变主招规则；ring、spread、summon、dash 只作为低频背景压力。新增 Boss 要提供敌人体格、纹理、BossSpec、BossVisual 美术规格与地图引用。

设计规则：

- 独占招式的反制只依赖移动与站位，不新增战斗中说明文字。
- 预警以 0.7 秒为基准，二阶段不低于 0.55 秒。
- 不以召唤物作为 Boss 核心记忆点；召唤仅作辅压。
- Boss 战不锁场，地图机制继续运行，因此高威胁招式需要错峰。
- Boss 美术标准与角色同级：程序化渐变、清晰五官、主题饰件、P2 强化层、主招/辅招施法姿态、受击反馈和童话谢幕死亡表现。
- Boss 预警不能使用裸线条；危险边界必须带 Boss/地图主题纹样，同时保留明确安全区和低遮挡透明度。
...
掉落道具定义在 `src/content/dropItems.ts`，效果在 `src/systems/dropItems.ts`。当前 43 个：7 通用 + 36 地图专属。
...
- 地图专属强杀伤道具用 `tags: ['lethal']` 标记，权重低于常规专属道具，定位为稀有高光。
