...
**当前进度：M7 已完成（16/16/16/8/8 全量达成——武器 13-16 补 whip/炮射/追踪/哨塔：卷卷藤/莓果弹弓/流萤珠/喇叭花号手 + 进化；被动 12→16：飘飘羽/莓果蜜饯/草叶哨/小花架；角色 13-16：藤藤/莓莓/悠悠/嘟嘟 + 4 新饰件；地图 6 莓果灌丛 26min（荆棘地皮 brambles）/地图 7 星语夜原 28min（流星雨 starfall）/地图 8 破晓之巅 30min 终局图（晨光柱 dawnpillar）；行为模板 14→16：burrow 钻地/phase 月相变速；makeEnemy 形体 15→18：eared/starlet/crescent；Boss 6 莓刺熊王/Boss 7 星角鹿王/Boss 8 永夜枭；成就 28→40 + 地图解锁链 lavenderClear→bramble→nocturne→summit）。下一步 M8：设置/调试完善 + 性能与平衡 pass。**
...
| M8 | 设置/调试完善 + 性能与平衡 pass | ⬜ 下一步 |
| M9 | 规则卡 Arcana（10 张）+ 收尾 | ⬜ |

**M8 注意**：调试面板补齐（武器 DPS 统计/波次预览）；FPS 采样动态敌人上限（图 6-8 终局在场 285-320，移动端压力最大）；纹理生命周期（离图 `textures.remove` 尚未实施，8 图全量纹理常驻）；全比例回归 + 数值平衡巡检（新四武器/被动/三图与既有内容横向对比）；BGM/SFX 分轨音量；4x CPU throttle 峰值 ≥45 FPS、全 8 图无软锁为验收线。
