...
src/content/   ★ 纯数据层（无 Phaser）：ids player characters(16 角色 Spec) weapons(16 武器含平衡表) passives(16)
               ...
               bosses(BossSpec 配装表×8) shop(15 项永久强化) achievements(46 成就+LEGACY 7+check+unlockChar/unlockMap/unlockArcana/rewardCoins)
               arcana(16 规则卡 META 含 tier 基础/机制+获取规则 ARCANA+数值 ARC_FX) affixes(M15 精英词缀 AFFIX 调参+AFFIX_COLOR 标识色)
...
| M14 | 构筑深度 II：5 角色专属 trait（wisp/berry/toot/ivy/jingle）+ 进化引导三件套（选卡角标/图鉴配方/首局 tips）（详见 docs/发行补强开发方案-M10-M15.md 第七章） | ✅ |
| M15 | 敌人行为扩展与精英词缀：3 新行为模板（exploder/shielder/summoner）+ 6 新敌人图 4-8 后段入池 + 5 精英词缀（狂暴 II/无尽 2 轮起）+ 剪影强化 + 2 新成就（详见 docs/游戏内容优先开发方案-M12-M20.md 第七章） | ✅ |
| M16 | 秘密内容：2 隐藏角色（blobby/nova，secret 不占位）+ 2 彩蛋链（草甸发光花圃/夜原流星雨反用）+ 隐藏成就（hidden ？？？行）+ 揭示动效（详见 docs/游戏内容优先开发方案-M12-M20.md 第八章） | ✅ |

**M1-M11 全部完成**：1.0 内容齐整（16 角色 / 8 地图 / 16 武器+16 超武 / 16 被动 / 67 敌 / 44 成就 / 10 规则卡，图鉴六类全覆盖）+ 通关后内容两层（无尽 / 狂暴）。

M10 要点：升级三选一支持重抽（R）/放逐（B；M16.5 起改为与重抽/跳过同排的按钮，进入选卡放逐模式，含宝箱升级层整局生效）/跳过（S），次数 = 1 + 商店等级；复活（`RunState.revivesLeft`，半血归来 + 2 秒无敌 + 清屏脉冲——计入击杀但经 `PickupSystem.suppressDrops` 总闸抑制一切掉落，含规则卡钩子产币，防送死换收益）；商店 15 项（总池 9,906，前 1,530 价格曲线一个数字不动）；公共契约 `RunLaunchData` / `RunResult.mode/diff/cycle` 已在 `systems/context.ts` 预留（M11 实装）。纯数据层单测：`npm test`（vitest，覆盖价格曲线/总池/存档 sanitize 宽容性——调经济参数后须同步 `tests/shop.test.ts` 的口径数字）。
...
M14 要点（构筑深度 II，docs/发行补强开发方案-M10-M15.md 第七章 + 新方案第六章差异）：**角色 trait** = `CharacterSpec.trait?: TraitId`（ids.ts 新联合类型）+ 数值表 `TRAIT_FX`（content/characters.ts，调参只改 content）+ 行为 `systems/traits.ts` 的 `createTraitModifier(id, ctx, weapons)`（纯函数注册表，完全复用 M13 RunModifier 钩子，M16 隐藏角色直接复用）；`GameScene.create` 在一切 arcana 之前 push（其余 11 角色零开销缺省路径，bench 不挂）。五机制：wisp 轻盈残影（modifyPlayerDamage 就绪返 0 + 朝 facing 闪现 90px 留残影；冷却 12s 经 `run.flickerCdLeft` 暴露给 HUD 就绪点——HP 条右侧小圆点；iframe/调试无敌判定在前，不消耗闪避）/ berry 甜食小铺（onCoinPicked 每 8 枚在最近敌人处果酱爆炸 30×dmg + slow 果酱区；周围无敌人保留计数待发）/ toot 晨光大合奏（onTick 每 40s 全武器 `triggerNow()`——Weapon 基类新增，仅清零 cdT 下帧自然 fire，哨塔/地雷池上限照常；SFX.fanfare 号角）/ ivy 收集家（statMods offers=4，M13 字段消费；HUD 补 keydown-FOUR）/ jingle 进化共鸣（onEvolve 随机未满级被动 +1（ctx.rng）+ 钟鸣新星 40×dmg r220）。trait 衍生伤害一律 `noHook + dmgLog('trait_*')`。**UI** = CardOpts 新增 `subDesc/subColor`（column 布局卡底金句）：角色选择/图鉴角色页 `✦ 机制名`（图鉴 desc 换 trait 全文，未解锁仍 ???）；开局金色 toast 宣告（`elapsed<0.5` 守卫防设置页返回重复）。**进化引导** = ① 选卡角标 `PickCardInfo.evoHint`（武器卡：toLevel=5 且配对被动已持有→金色「开宝箱可进化！」，否则灰字「满级 + {被动} 可进化」、mine 通配文案；被动卡：恰为已持有未进化武器配方→「与「{武器}」配对进化」；竖屏条卡 ch≥96 才显示）② 图鉴武器页 subDesc `⇐ 配方`（evoHintNeed/evoHintAny 复用零新键）③ `systems/TipSystem.ts`（RunSystem，0.5s 节流扫描）：精英首现后 2s + 武器首达满级各 toast 一次，节流 `save.tipsSeen`（M11 v2 搭车字段，Meta.hasTip/markTip），展示走 `hud:tip` 事件 → queueToast 停留 3.5s（toast 队列改 `{text, hold}`）。**顺手修复**：HUD `toastQueue/toastBusy` 未随场景重启复位——上局 shutdown 杀掉在播 toast 的 tween 后 busy 卡 true，新局一切金色横幅永久哑火；create 起手复位。check-i18n 增 TraitId→`trait_<id>(_d)` 推导规则；i18n +18 键。

M15 要点（敌人行为扩展与精英词缀，docs/游戏内容优先开发方案-M12-M20.md 第七章 + 7.5 差异记录）：**3 新行为模板**（behaviors.ts 16→19，调参常量 EXPLODER/SHIELDER/SUMMONER 在 content/enemies.ts）——`exploder` 自爆（加速逼近 → r110 定身 1s 膨胀预警（`Effects.teleCircle` 半透明警示区+收缩描边，M12 规范）→ 爆炸 r130：对玩家 spec.dmg（随 dmgScale）/ 对敌固定 60 可借力清群；被击杀半径减半再爆，`Enemy.exploded` 防重复，结算函数 `behaviors.exploderBoom` 共用）/ `shielder` 护盾光环（每 0.5s 重算 320px 敌群重心慢速跟随；光环 r140 友方减伤 35%——`EnemySystem.shieldMulFor` 在 `Game.hitEnemy` 乘入，护盾怪列表 0.3s 重建，自身不受庇护；光环常显圈 `Enemy.auraImg` 随池复用）/ `summoner` 召唤（保持 260 距离踱步，每 6s 吟唱 0.6s 召 2 只 `EnemySpec.summon` 指定杂兵；单体存活 cap 6（`summonerRef` 归属计数，release 清引用）+ 340 全局硬上限双约束）。**6 新敌人后段入池**（图 4-8 波次表后 50% 段，前 3 图逐字节不变）：bombcap(grove,700s)/hivebud(lavender,680s)/husker(bramble,700s)/novamote(nocturne,880s)/duskward(summit,860s)/shadowmaw(summit,1230s)，换皮配方进 mapassets、配色进 palette 图色组+DEATH_COLOR，图鉴敌人 +6 自动展示。**精英词缀**（`content/affixes.ts`：AffixId 进 ids.ts，AFFIX 调参 + AFFIX_COLOR 标识色）：狂暴 II 全部精英 / 无尽第 2 轮起精英（含 surge/护卫）经 `EnemySystem.onEnemySpawned` 后置钩子（3.2 契约 #3）随机挂 1 个（`ctx.rng()`，M17 种子契约）——swift 移速×1.35+dash recover×0.6（`Enemy.recoverMul`）+残影 / bulwark HP×1.3+knockMul×0.2+体型×1.15 / splitting 死亡裂 4 只本图基础杂兵（首波首项口径）/ gravitic 每 5s 拉拽玩家 0.5s 力度 120 限 460px / volley 每 4s 环形 6 弹（蓄力闪烁预警）；词缀名牌为世界空间 Text 浮签（EnemySystem 维护随池复用，HUD 零改动）。**剪影强化**：高威胁行为（dash/turret/ambush/blink+新三类）`baseScale` +10%（视觉口径碰撞不变），新敌人警示向专属描边色。**2 新成就**：affixSlayer（跨局累计 20 词缀精英，`stats.affixKills` v2 搭车字段零迁移，`recordRun` 入账含中途退出）/ graviticEscape（狂暴 II 通关且 gravSeen 真、gravHit 假——`damagePlayer` 扩 `src?: Enemy` 埋点，接触伤害从 PlayerSystem 传来源）。RunResult 扩 affixKills/gravSeen/gravHit；check-i18n 增 `affix_<id>` 推导；i18n +15 键。验收实测：五词缀齐现、护盾×0.65 圈内/击杀立即失效、召唤 cap 双约束、340 同屏+3 护盾怪 120 FPS。

M16 要点（秘密内容，docs/游戏内容优先开发方案-M12-M20.md 第八章 + 8.3 差异记录）：**2 隐藏角色** = `CharacterSpec.secret`（未解锁时 CharacterSelect/Codex **不占位不显示**——与普通 ??? 占位区分，存在本身就是秘密；解锁后插列表尾部 16→18，TARGET 占位数对 `visible.length` 计算）——blobby 小蓝团（lantern 起手 / hp160 spd150 r16 / area1.1 dmg0.92 / trait `bouncy` 软软弹弹：受击把来源敌人弹开 kb240×knockMul 不减伤）+ nova 小流星（star 起手 / hp78 spd200 r12 / projSpeed1.2 cd0.9 dmg0.92 / trait `comet` 流星之躯：持续移动 0.8s 后受伤 ×0.9，位移阈值=期望步长三成防抖）。trait 全走 M14 `createTraitModifier` 注册表 + TRAIT_FX 调参；**bouncy 需要伤害来源 → `modifyPlayerDamage` 钩子扩第三参 `src?: Enemy`**（Game.damagePlayer 透传，存量钩子零改动）。纹理走 makeCharacter 管线（保 4 帧动效与角色/敌人美术分界，**不直接复用 e_blob/p_star 位图**，只取其配色意象：CHAR_PAL blobby 蓝团/nova 彗星金）。**2 彩蛋链**：草甸发光花圃（`GameScene.spawnSecretBloom`：35%/局、离出生点 600-900px、`sb_glow` 微光纹理脉动；玩家 320px 内首次靠近轻响提示，70px 内站立 5s `bloomSecret` 绽放 → `run.bloomed`）；夜原流星反用（MapMechanicSystem.impact 玩家圈内 `run.meteorHits++`，无敌帧不豁免计数）。**隐藏成就** = `AchievementSpec.hidden`：secretBloom→blobby、stargazer（被砸 3 次仍存活）→nova；成就页未达成时**条件/奖励也不显示**（`ach_hiddenDesc` 通用文案；标题 ？？？ 本就由 Card locked 态渲染，零新结构），计入总数 46→48；「仍存活」语义由 AchievementTracker 逐秒评估天然承担，**Result 终评传 bloomed:false/meteorHits:0 不补评**。**揭示动效** = CardGrid `reveal` 选项（卡片横向翻转入场 + 10 枚星屑迸发，星屑经 add() 进内容容器随滚动）；触发凭据 `Meta.markTip('reveal_<id>')`（tipsSeen 搭车，全存档一次，resize 重建不复播）。AchRunView 扩 bloomed/meteorHits；RunState 扩同名埋点；零存档迁移（角色解锁走成就 unlockChar 既有链路，Boot.syncAchUnlocks 自动回填）。i18n +13 键（角色 2×2 + trait 2×2 + 成就 2×2 + ach_hiddenDesc；花圃彩蛋零文案——发现引导只靠微光与轻响）。
