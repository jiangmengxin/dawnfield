...
**当前进度：M5 已完成（MapSpec 全链路 ×3 图、行为模板 4→12、makeEnemy 换皮管线+ensureMapAssets 懒生成、地图 2 露珠池塘 15min/地图 3 晚霞山岗 18min、MapMechanicSystem 减速水皮/定时大风、BossSpec 配装表参数化 BossController+Boss 2-3、timeK 长图成长缩放、倍速穿隧半步判定、地图解锁链 unlockMap、每图 BGM 主题、check-i18n 脚本挂 build 链）。下一步 M6：内容批次 B（角色/武器 9-12、地图 4-5、被动→12、成就→28）。**
...
src/content/   ★ 纯数据层（无 Phaser）：ids player characters(8 角色 Spec) weapons(含平衡表) passives
               enemies(26 敌 Spec/12 行为指派+调参) maps(MapSpec 全链路×3：波次/装饰/机制/BGM/解锁)
               bosses(BossSpec 配装表) shop(11 项永久强化) achievements(14 成就+check+unlockChar/unlockMap)
...
src/systems/   context(CombatContext/RunSystem/RunModifier) WaveDirector(map 驱动) EnemySystem behaviors(12 模板)
               BossController(BossSpec 数据驱动) MapMechanicSystem(水皮/大风)
...
src/gfx/       palette(全游戏配色+CHAR_PAL+POND/HILLS 图色组+DEATH_COLOR) textures/(core｜characters+makeCharacter
               配方｜enemies(草甸)｜mapassets(makeEnemy 换皮管线+每图敌人/装饰/弹体+ensureMapAssets)｜weapons｜icons｜misc)
...
- **地图机制**：`MapMechanicSystem` 按 `MechanicSpec` 二选一——`puddles`（周期水皮，`ZoneSpec.affectsPlayer` 让玩家也减速，PlayerSystem 接 `ctx.playerSlowAt`）/`storm`（预警→定时推挤，敌人按 knockMul 受风）。新机制在此文件加分支。
...
| M6 | 内容批次 B（9-12、地图 4-5） | ⬜ 下一步 |
| M7 | 内容批次 C（13-16、地图 6-8）补完 | ⬜ |
| M8 | 设置/调试完善 + 性能与平衡 pass | ⬜ |
| M9 | 规则卡 Arcana（10 张）+ 收尾 | ⬜ |

**M6 注意**：武器 9-12 补 zone/orbit/melee/burst 机制空缺（新武器 = weapons/ 行为文件 + content 平衡表 + FACTORY 注册 + 图标 + i18n 四键）；角色 9-12 配对新武器（一行 Spec + 一行 makeCharacter 配方）；地图 4-5 走 M5 管线（MapSpec + ensureMapAssets 纹理组 + BossSpec + i18n 四键），机制为治愈泉（ZoneSystem heal 已支持）/麦浪阵风；行为模板 12→14；成就→28（解锁条件兼新内容）；每批次跑 `npm run check:i18n`。
