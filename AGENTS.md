...
**当前进度：M2 已完成（content 数据层、Game.ts 拆 8 系统、CombatContext、行为模板表、宝箱分层、RunModifier 空挂）。下一步 M3：存档/金币/解锁/成就/图鉴骨架。**
...
src/core/      events(类型化事件) router settings RunState TimeController registry input/(键盘/触控/手柄stub)
src/ui/        Viewport(安全区/断点/缩放) UIScene(菜单基类) layout theme widgets/(8 组件)
src/scenes/    Boot Title CharacterSelect MapSelect Shop Codex Achievements Settings Game(249行编排器) HUD Result
src/systems/   context(CombatContext/RunSystem/RunModifier) WaveDirector EnemySystem behaviors BossController
               weapons/(基类+7武器) PlayerSystem PickupSystem ProjectileSystem ZoneSystem LevelUpSystem
               DecorSystem grid effects joystick
```
...
- **宝箱分层**（`LevelUpSystem.buildChestReward`）：可进化→进化；否则→已持有项升级×N（`CHEST.upgradeCount`）；无可升级→金币（M3 前为经验+治疗）。
- **局内状态**全在 `core/RunState`（hp/xp/kills/passives/stats…），HUD 经 `gs.run` 读取；新系统实现 `RunSystem` 并在 `GameScene.create` 的 systems 数组注册（顺序即帧序）。
...
| M3 | 存档/金币/解锁/成就/图鉴骨架 + 调试面板完善 | ⬜ 下一步 |
...
**M3 注意**：存档 schema 改动必配迁移并用旧档验证；现有 `dawnfield.lang/muted/volume/settings` 键作 v0 吸收进 `dawnfield.save`；金币掉落挂在 `PickupSystem`，结算入账走 `RunResult`。
