...
**当前进度：M8 已完成（设置/调试完善 + 性能与平衡 pass——BGM/SFX 分轨音量（volBgm/volSfx，旧档 volume 作两轨默认值吸收）；武器 DPS 统计（DpsTracker + WeaponManager 归账子上下文 + ZoneSpec.src，HUD 调试覆盖层显示）；波次预览（HUD 调试行 + 设置页调试弹窗）；FPS 采样动态敌人上限（dynCapMul 0.4-1.0 + 粒子降档）；纹理生命周期（进图 releaseMapAssets 释放其它图懒生成纹理）；ScrollPanel 遮罩修复（Phaser 4 WebGL 改用 Mask filter）；平衡巡检（16 武器单武器 Lv5×30s 实测，上调流萤珠/喇叭花/蒲公英）；验收：4x throttle 折算峰值 ≥45 FPS、8 图全链路通局零软锁）。下一步 M9：规则卡 Arcana（10 张）+ 收尾。**
...
| M9 | 规则卡 Arcana（10 张）+ 收尾 | ⬜ 下一步 |

**M9 注意**：`RunModifier` 六钩子（statMods/modifyOffers/onEnemyKilled/modifyDamage/onChest/onTick）自 M2 起已在全部调用点空挂，实装只需向 `GameScene.modifiers` 推卡；ArcanaSpec 进 content/arcana.ts（纯数据），i18n 键 `arc_<id>(_d)`；选卡 UI（开局选 1 + Boss 宝箱可再得）与规则卡图鉴页；验收线：10 卡可叠加无冲突、关闭规则卡开关行为与 M8 等价（卡牌数值可用 M8 的单武器 DPS 基准法实测校准）。
